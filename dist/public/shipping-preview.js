...
(async function initSkuEstimator() {
  ...
  function addRow(skuVal, qtyVal) {
    ...
    tr.innerHTML = '<td><div class="sku-input-wrap">' +
      ...
      '<td><button class="sku-remove-btn" title="Remove">x</button></td>';
    ...
    inp.addEventListener('focus', function() { showAC(inp, dd); });
    inp.addEventListener('blur', function() { setTimeout(function() { dd.style.display = 'none'; }, 150); });
    inp.addEventListener('keydown', function(e) { if (e.key === 'Tab' || e.key === 'Enter') dd.style.display = 'none'; });
    tr.querySelector('.sku-qty-input').addEventListener('input', updateTotals);
    tr.querySelector('.sku-remove-btn').addEventListener('click', function() { tr.remove(); updateTotals(); });
    updateTotals();
    if (!skuVal) inp.focus();
  }

  addRow();
  document.getElementById('skuAddRow').addEventListener('click', function() { addRow(); });
  document.getElementById('skuClearAll').addEventListener('click', function() { document.getElementById('skuCartBody').innerHTML = ''; addRow(); });

  document.getElementById('skuRunPreview').addEventListener('click', async function() {
    ...
    try {
      ...
      if (data.shipment) {
        ...
        out += '  Total qty:       ' + lines.reduce(function(a,l){return a+l.quantity;},0).toLocaleString() + '\n';
        out += '  Net weight:      ' + (s.totalNetWeightLb || 0).toFixed(4) + ' lb\n';
        out += '  Shipment weight: ' + (s.totalShipmentWeightLb || 0).toFixed(4) + ' lb (incl. tare: ' + s.tareLb + ' lb)\n\n';
        out += 'Line Details\n';
        (s.lines || []).forEach(function(line, i) {
          const label = (getRows()[i] && getRows()[i].querySelector('.sku-input').value) || ('Line ' + (i + 1));
          out += '  ' + label + ': qty=' + line.quantity + ', ' + line.trueWeightGrams + 'g/unit -> ' + (line.resolvedWeightLb || 0).toFixed(4) + ' lb\n';
        });
      }
      if (data.rates && data.rates.length) {
        out += '\nCarrier Rates\n';
        data.rates.forEach(function(r) { out += '  ' + r.carrier.toUpperCase().padEnd(8) + ' ' + r.serviceName.padEnd(30) + ' $' + (r.amountUsd || 0).toFixed(2) + ' ' + r.currency + '\n'; });
      }
