    ...
    try {
          ...
          if (res.rows.length > 0) {
            ...
            const dp = defaultSettings.packaging;
                  ...
                  // skuBoxOverrides is source-of-truth in defaultSettings.ts — always sync
            // from code so a stale DB value (e.g. old "6-W" rule) can never resurface.
            if (JSON.stringify(_cachedSettings.packaging.skuBoxOverrides) !== JSON.stringify(dp.skuBoxOverrides)) {
                      _cachedSettings.packaging.skuBoxOverrides = JSON.parse(JSON.stringify(dp.skuBoxOverrides));
                      console.log('[settingsStore] migrated packaging.skuBoxOverrides from defaults');
            }
